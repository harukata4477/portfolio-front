  ...
  created() {
    this.$axios.$get('api/users/me', {
      ...
      if(res.id == this.$route.params.id){
        this.items = [{title: `ユーザー情報`, link: `/users/${this.$route.params.id}`},{title: '投稿一覧', link: `/users/posts/${this.$route.params.id}`},{title: `フォロワー一覧`, link: `/users/follows/${this.$route.params.id}`},{title: `設定`, link: `/users/edits/${this.$route.params.id}`}]
      }else{
        this.items = [{title: `ユーザー情報`, link: `/users/${this.$route.params.id}`},{title: '投稿一覧', link: `/users/posts/${this.$route.params.id}`},{title: `フォロワー一覧`, link: `/users/follows/${this.$route.params.id}`}]
      }
    })

    this.$axios.$get(`api/users/likes/${this.$route.params.id}`).then(res => {
  ...
  methods:{
    back(){
      if(this.first_page == true){
        ...
        this.nowPage--
        this.$axios.$get(`/api/users/likes/${this.$route.params.id}?page=${this.nowPage}`).then(res => {
          this.targets = res.post_targets
          this.posts = res.posts
          this.rooms = res.rooms
          this.users = res.users
