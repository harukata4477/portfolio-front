...
<script>
  function setupDiagram(divname) {
    var $ = go.GraphObject.make;
    var myDiagram = $(go.Diagram, divname, { "undoManager.isEnabled": true });
...
console.log(node.data);</code></pre>
...
<p>
  ...
  search through the node for the named object. It will return a reference to
  the named GraphObject if it is found, or <code>null</code> otherwise.
</p>
...
<button id="selectionButton">
  Select some Nodes; then click here to change Shape.fill inside selected
  Nodes
</button>
<script>
...
console.log(node.data.color);</code></pre>
...
// and the user will see the red node</code></pre>
...
<ul>
  <li>
    ...
    <a href="../intro/dataBinding.html"
      >intro page on Data Binding for lots more detail</a
    >.
  </li>
  <li>
  ...
  <li>
    ...
    <a
      href="https://developers.google.com/chrome-developer-tools/docs/console"
      >Chrome guide</a
    >
    or the
...
<p>
  ...
  tutorials on
  <a href="https://www.youtube.com/channel/UC9We8EoX596-6XFjJDtZIDg"
    >YouTube</a
  >.
</p>
<p>
