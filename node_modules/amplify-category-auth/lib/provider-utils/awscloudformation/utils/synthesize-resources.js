...
const getResourceUpdater = (context, cfnFilename, provider) => async (request) => {
    ...
    const adminQueriesFunctionName = lodash_1.get(resources, ['api', 'AdminQueries', 'dependsOn'], [])
...
const updateUserPoolGroups = async (context, resourceName, userPoolGroupList) => {
    if (userPoolGroupList && userPoolGroupList.length > 0) {
        ...
        const userPoolGroupFile = path.join(context.amplify.pathManager.getBackendDirPath(), 'auth', 'userPoolGroups', 'user-pool-group-precedence.json');
