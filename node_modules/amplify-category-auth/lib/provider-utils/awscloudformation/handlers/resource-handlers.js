...
const getAddAuthHandler = (context) => async (request) => {
    ...
    await synthesize_resources_1.getResourceSynthesizer(context, cfnFilename, provider)(requestWithDefaults)
        ...
        .then(amplify_meta_updaters_1.getPostAddAuthMetaUpdater(context, { service: requestWithDefaults.serviceName, providerName: provider }))
        .then(message_printer_1.getPostAddAuthMessagePrinter(context.print))
        .catch(err => {
...
const getUpdateAuthHandler = (context) => async (request) => {
    ...
    await synthesize_resources_1.getResourceUpdater(context, cfnFilename, provider)(requestWithDefaults)
        .then(req => req.resourceName)
        .then(amplify_meta_updaters_1.getPostUpdateAuthMetaUpdater(context))
        .then(message_printer_1.getPostUpdateAuthMessagePrinter(context.print))
        .catch(err => {
        context.print.info(err.stack);
